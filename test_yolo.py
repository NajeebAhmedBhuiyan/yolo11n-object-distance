...
def main():
    ...
    while True:
        ...
        # draw vertical image center line (for horizontal error reference)
        mid_x = img_w // 2
        cv2.line(frame, (mid_x, 0), (mid_x, img_h), (0, 255, 255), 1, lineType=cv2.LINE_AA)
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            for (xyxy, conf, classidx) in zip(xyxy_all, confs, clss):
                ...
                person_label = f'person  {conf*100:.0f}%'
                ...
                # --- centroid & horizontal error ---
                cx = (xmin + xmax) // 2
                cy = (ymin + ymax) // 2

                err_px = cx - (img_w // 2)           # + right, - left
                err_pct = err_px / float(img_w)      # fraction of image width

                # centroid dot
                cv2.circle(frame, (int(cx), int(cy)), 3, color, -1, lineType=cv2.LINE_AA)

                # horizontal error line (centerline to centroid at same y)
                line_y = int(cy)
                cv2.line(frame, (img_w // 2, line_y), (int(cx), line_y), color, 2, lineType=cv2.LINE_AA)

                # compact position/error label
                pos_label = f'({cx},{cy}) ex:{err_px:+d}px {err_pct:+.1%}'
